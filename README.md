...
## 🧩 Claude Code Skills & MCP Plugins

This repo uses Claude Code **custom skills** backed by two **MCP plugins** to keep research and library lookups grounded in live sources instead of stale training data.

### MCP Plugins

| Plugin | Provides | Used For |
|--------|----------|----------|
| **Exa** (`mcp__exa__*`) | Neural/semantic web search (`web_search_exa`, `web_fetch_exa`) | Finding authoritative, less SEO-optimized sources during topic research |
| **Context7** (`mcp__plugin_context7_context7__*`) | Live, version-accurate library documentation (`resolve-library-id`, `query-docs`) | Pulling current API signatures, code examples, and migration notes for open source libraries (e.g. LangChain, LangGraph, Deep Agents) |

Both are configured as MCP servers/plugins in Claude Code and are auto-discovered by skills at runtime via `ToolSearch` — no hardcoded tool names, so they degrade gracefully (falling back to `WebSearch`) if not connected in a given session.
...
#### `research-topic`
Researches a topic in depth by combining **Exa** and **WebSearch**, cross-checking conflicting claims between engines, and producing one synthesized, sourced write-up rather than a raw dump of search results. Invoke with `/research-topic <topic>`.

#### `opensource-documentation`
Looks up current, version-accurate documentation for a named open source library, framework, SDK, or CLI tool via **Context7**. Resolves the library ID, queries docs scoped to one concept at a time, and always cites the returned source URLs — falls back to `WebSearch` against official docs if Context7 isn't connected. Invoke with `/opensource-documentation <question>`.

**Example — used in this repo to answer "how to create deep agents in LangChain" and "recent code to build agents in LangChain"**, pulling directly from `reference.langchain.com` via Context7 rather than relying on potentially outdated knowledge of the `create_agent` / `create_deep_agent` APIs.

### Setting Up Exa MCP Locally

```bash
claude mcp add exa -e EXA_API_KEY=your_key_here -- npx -y exa-mcp-server
